...
//        Set<Manager> mgrSet = map.keySet(); // all the managers in map
        for (Manager mgr : map.keySet()) {
            if (mgr.getId() == id) {
                ...
                try {
                    // if that manager already has reportees
                    // append the new list of reportees
                    map.get(mgr).addAll(newReportees);
                }catch (NullPointerException e){
                    // manager has no reportees, i.e, list of reportees is null
                    // create and put the new list of reportees to this manager
                    map.put(mgr, newReportees);
                }
